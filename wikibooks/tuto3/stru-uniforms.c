...
int init_resources()
  ...
  struct attributes triangle_attributes[3] =
    {
      { .coord2d = { 0.0, 0.8 }, .color = {1.0, 0.5, 0.0, } },
      { .coord2d = {-0.8,-0.8 }, .color = {1.0, 1.0, 0.0, } },
      { .coord2d = { 0.8,-0.8 }, .color = {1.0, 0.0, 0.0, } },
    };
  GLint compile_ok;
  ...
  if (!(vs = create_shader("vertex.glsl", GL_VERTEX_SHADER)))
    return 0;
  if (!(fs = create_shader("unif-fragment.glsl", GL_FRAGMENT_SHADER)))
    return 0;
  ...
  if (!link_ok)
    {
    fprintf(stderr, "couldn't link program\n");
    print_log(program);
    return 1;
    }
  ...
  if (attribute_coord2d == -1)
    {
    fprintf(stderr, "couldn't bind coords\n");
    return 1;
    }
  ...
  if (attribute_v_color == -1)
    {
    fprintf(stderr, "couldn't bind color\n");
    return 1;
    }
  ...
  if (uniform_fade == -1)
    {
    fprintf(stderr, "couldn't bind fade\n");
    return 1;
    }
